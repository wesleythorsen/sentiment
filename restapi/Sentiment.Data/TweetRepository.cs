    ...
    public class TweetRepository : ITweetRepository
    {
        ...
        public Web.Api.Models.Tweet[] GetTweetsByConfigId(long configId, DateTime startDate, DateTime endDate, int limit = int.MaxValue)
        {
            ...
            var sqlTweets = _TwitterEntities.tweets
                .AsExpandable()
                .Where(t => t.created_at >= startDate)
                .Where(t => t.created_at < endDate)
                .Where(predicate)
                .Take(limit)
                .ToArray();
        ...
        private static Web.Api.Models.Tweet ToTweet(MySql.tweet sqlTweet)
        {
            ...
            {
                Id = sqlTweet.id,
                CreatedAt = sqlTweet.created_at,
                UpdatedAt = sqlTweet.updated_at,
                RetweetCount = sqlTweet.retweet_count,
                FavoriteCount = sqlTweet.favorite_count,
                UserId = sqlTweet.user_id,
                Text = sqlTweet.text,
                SourceId = sqlTweet.source_id,
            };
        }
    }
}
